...
Since the eigen function returns unitary vectors we need to scale them by sqrt(93)... Perhaps 93 was used because that is the number of observations so I should use 1076
...
par(mfrow=c(2,1))
plot(time_ms, e_fns[,k_pc], lty=1, lwd=2, type='l', ylim=c(-1,1),
...
for(i in 1:n){
  erp_scores[i,] <- colMeans(matrix(rep(erp_demeaned[i,], n_pc), nrow=1076) * e_fns)
  erp_fitted[i,] <- mean_hat + erp_scores[i,]%*%t(e_fns)
}

matplot(time_ms, t(mas_just_dat[sel.crv,]), pch = "o", cex = 0.5,
        ylab="", xlab="tract", main="Fitted curves")
matlines(time_ms, t(erp_fitted[sel.crv,]), type='l', lwd=2, lty=1)
```


```{r}
demeaned <- DTI.baseline$cca - t(matrix(rep(mean.hat, n),                                        nrow=length(mean.hat)))
