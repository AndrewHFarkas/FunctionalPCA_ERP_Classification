...
for (sub_index in 1:number_of_subs) {
  ...
  for (fold_index in 1:3) {
    ...
    for (channel in 1:channels){
      ...
      current_dat_one_chan <- current_dat %>% filter(electrode == channel)
      
      just_volts_over_time <- 
        dat_one_channel[, 8:(ncol(current_dat_one_chan)-1)] #minus one to exclude fold_id
      ...
      train_dat <- current_dat_one_chan %>% filter(fold_id != fold_index)
      
      test_dat <- current_dat_one_chan %>% filter(fold_id == fold_index)
